...
export default function DetailModal() {
  ...
  // ── DATASETTEN GELEN VERİLER ──
  const title = (params.title as string) || "İsimsiz Ürün";
  const price = (params.price as string) || "Fiyat Belirtilmedi";
  const imgUrl = (params.imgUrl as string) || "https://images.unsplash.com/photo-1555041469-a586c61ea9bc";
  const location = (params.location as string) || "Konum Belirtilmedi";
  
  // Sabit veya Fallback Alanlar
  const condition = (params.condition as string) || "İkinci El (Temiz)";
  const sellerName = (params.seller as string) || "Kullanıcı";
  const rating = (params.rating as string) || "4.5";
  
  // 🔥 DATASETTE AÇIKLAMA YOKSA ÜRETİLECEK AKILLI ŞABLON METNİ:
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: '#f8eeee' },
  backBtn: { paddingVertical: 4 },
  backBtnText: { color: '#C85C5C', fontSize: 15, fontWeight: 'bold' },
  headerTitle: { fontSize: 15, fontWeight: 'bold', color: '#2d1515' },
  mainImage: { width: '100%', height: 280, resizeMode: 'cover', backgroundColor: '#f9f9f9' },
  detailsContent: { padding: 20, flex: 1, backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, marginTop: -20 },
  productTitle: { fontSize: 17, fontWeight: 'bold', color: '#333', marginBottom: 6, lineHeight: 22 },
  ...
  sectionTitle: { fontSize: 14, fontWeight: 'bold', color: '#2d1515', marginBottom: 10 },
  infoGrid: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 5 },
  infoBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fdf3f3', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, marginRight: 8, marginBottom: 8, borderWidth: 1, borderColor: '#fcdcdc' },
  badgeIcon: { fontSize: 13, marginRight: 4 },
  infoBadgeText: { fontSize: 12, color: '#555', fontWeight: '500' },
  descriptionBox: { backgroundColor: '#fafafa', padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#f3f3f3' },
  descriptionText: { fontSize: 13, color: '#555', lineHeight: 20 },
  sellerRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: '#f0e4e4' },
  sellerAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#fdebeb', alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 16 },
  sellerName: { fontSize: 13, fontWeight: 'bold', color: '#333' },
  sellerStatus: { fontSize: 11, color: '#777', marginTop: 2 },
  takipEtBtn: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#C85C5C', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  takipEtText: { color: '#C85C5C', fontSize: 11, fontWeight: 'bold' },
  pazarlikActionBtn: { backgroundColor: '#C85C5C', paddingVertical: 14, borderRadius: 12, alignItems: 'center', marginBottom: 20 },
