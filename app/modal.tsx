...
export default function DetailModal() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        console.error("Firebase'den ürün detayı çekilirken hata oluştu:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const title = productData?.Product_name || (params.title as string) || "İsimsiz Ürün";
  const price = productData?.Price ? String(productData.Price) : (params.price as string) || "Fiyat Belirtilmedi";
  const imgUrl = productData?.Image || (params.imgUrl as string) || "https://images.unsplash.com/photo-1555041469-a586c61ea9bc";
  const location = productData?.Address || (params.location as string) || "Konum Belirtilmedi";
  const condition = productData?.Condition || (params.condition as string) || "İkinci El (Temiz)";
  const sellerName = productData?.SellerName || (params.seller as string) || "Kullanıcı";
  ...
  const description = fetchedDescription || (params.description as string) ||
